...
# ---------------------------------------------------------------
def engineer_features(df: pd.DataFrame, return_display: bool = False) -> Union[pd.DataFrame, Tuple[pd.DataFrame, pd.DataFrame]]:
    """Prepare data for modeling

    If ``return_display`` is True the function will also return a copy of the
    dataframe *before* label-encoding so that the original categorical values
    can be rendered back to the user in the Streamlit UI.
    """
    ...
    # Preserve a copy for UI display before encoding
    df_unencoded = df.copy()

    # Label encode categorical variables for the model input
    le = LabelEncoder()
    for col in df.select_dtypes(include=["object", "category"]).columns:
        df[col] = le.fit_transform(df[col])

    if return_display:
        return df, df_unencoded
    return df
...
def make_predictions(raw_df: pd.DataFrame,
    ...
    df_encoded, df_display = engineer_features(raw_df.copy(), return_display=True)

    # 3-B  preprocessing + feature-selection
    X_clean = pipeline.transform(df_encoded)              # ndarray

    # 3-C  inference
    preds   = model.predict(X_clean)
    preds_label = np.where(preds == 1, "Yes", "No")

    # 3-D  assemble results
    result  = pd.concat(
        [pd.Series(preds_label, name="Predicted_Subscription"), df_display.reset_index(drop=True)],
        axis=1,
    )
...
def main():
    ...
    if st.button("Predict"):
        ...
        # quick counts
        counts = results["Predicted_Subscription"].value_counts().to_dict()
        st.info(f"Yes: {counts.get('Yes',0)} | No: {counts.get('No',0)}")
